...
class GetTableDesc(ToolsBase):
    ...
    def _get_table_comment(self, conn, db_type: str, table_name: str) -> str:
        try:
            if db_type in ['dameng', 'oracle']:
                sql = f"SELECT COMMENTS FROM USER_TAB_COMMENTS WHERE TABLE_NAME = '{table_name}' AND TABLE_TYPE = 'TABLE'"
            elif db_type == 'mysql':
                sql = f"SELECT IFNULL(TABLE_COMMENT, '') FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = '{table_name}' AND TABLE_TYPE = 'BASE TABLE'"
            elif db_type == 'postgresql':
                sql = f"SELECT COALESCE(obj_description(relfilenode, 'pg_class'), '') FROM pg_class WHERE relname = '{table_name}'"
            elif db_type == 'mssqlserver':
                sql = f"""
